...
// Only allow if NOT a donor
if (isset($_SESSION['is_donor']) && $_SESSION['is_donor'] == 1) {
    echo "<main class='content'><h2 style='text-align:center;color:red;'>You are registered as a donor. Please use the donor profile page.</h2></main>";
...
// Fetch user data
if ($userId) {
    $stmt = $conn->prepare("SELECT name, email, gender, phone, address, pin_code, city_id, country, created_at FROM users WHERE id = ?");
...
<script>
    $(document).ready(function() {
        ...
        $('#updateProfileForm').on('submit', function(e) {
            ...
            $.ajax({
                url: '<?= BASE_URL ?>/user/php_files/receiver/profile/update.php',
                type: 'POST',
                data: formData,
                dataType: 'json',
        ...
        $('#changePasswordForm').on('submit', function(e) {
            ...
            $.ajax({
                url: '<?= BASE_URL ?>/user/php_files/receiver/profile/change_password.php',
                type: 'POST',
                data: formData,
                dataType: 'json',
                success: function(res) {
